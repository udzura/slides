...
- **Google発の言語**（2009年公開）
  - C/Javaのような大規模開発の課題を、シンプルさで解こうとした言語
- 文法がかなりシンプル
  - クラスがない（structとinterfaceで組み立てる）
  - 例外がない（戻り値で `error` を返す）
  - Genericsもつい最近 (1.18, 2022) まで無かった
- ビルドが高速、ワンバイナリ化が容易
  - libc非依存でランタイムを内包
- 並行処理が言語の第一級の概念
  - 軽量コルーチン **Goroutine** と **channel**
...
- `go` で関数を別スレッド（goroutine）として起動
- `chan` で値の受け渡し・同期
- 「並行処理は最初から書ける前提」の言語設計
...
- バックエンド・ミドルウェア開発に必要なものはほぼ標準
- ある意味、**Go自体がミドルウェア開発のフレームワーク**
  - サードパーティに頼る前にまず標準を見る文化
...
- 重厚なフレームワークを使わない文化が強い印象
  - 依存ライブラリも最低限が推奨されがち
- ドメインに関するコードは、プロジェクトごとに**自分たちで設計**
  - レイヤードアーキテクチャ／DDDの素養があるメンバーがいないと、
    なかなかすごい設計になることも...
- 「普通に書くのが一番速い」設計はとても便利
...
- とにかく **全部揃った状態** でスタートできる
  - テスト・非同期ジョブ・CI・認証...「定番」がある
- 自分で選択する機会が減る
  - メリットでもデメリットでもある
- コミュニティ想定外の用途になると...
  - 結局「普通のプログラミング」になり、地力が出る

→ どちらが優れている、ではなくドメインとの相性
...
- Goは **シンプル＆並行処理に強い** 言語
- バックエンドでは「自分で設計する」覚悟があれば刺さる
- 「細かいところまで設計し切るぞ」という気持ちと相性◎
- ミドルやインフラに近いところを書きたい人に特におすすめ
...
- もしくは **全体設定を今のベスト** にしたうえで
  - どれくらいの **スループット (rps)** が出るかを計測する
...
- Pythonの DSL でシナリオを書ける負荷試験フレームワーク
- 自分のチームでは主にこれを使っている
- メリット
  - マルチワーカー (master/worker) の試験がわかりやすい
  - シナリオの記述が素直
- デメリット
  - マルチワーカーゆえに設定の反映や再起動がわかりづらい
  - 個人的には **分析が弱め** な印象（CSV落として頑張りがち）
...
- 機能面・安定性は k6 が一歩リードという印象
  - デメリットだったGUIの不在も、最近Web UIが出てきた
  - ただし自チームとしては、 Locust の **既存資産** があるので、移行は後回し中...
...
-  **1. 仮想ユーザー（Virtual User / VU）とは**
    *   実際の人間ではなく、サーバーへリクエストを送る **「自動化されたプログラム（ロボット）」**
...
<!--
補足ですが、仮想ユーザーっていうのはロボットみたいなもので、シナリオ通りに動いて、待ち時間を挟んで、ループする。これが基本の動きです。
（15秒）
-->
...
- **サーバ**: Go + echo + PostgreSQL のミニ TODO アプリ
  - AIにベースを書いてもらう
- **クライアント**: k6 で CRUD をぐるぐる
...
- 前回の試験を踏まえて、**やれていないシナリオ・状況** を確認
- どういう設定変更を反映させるか、**スコープを絞る**
  - 「全部一気に変える」と何が効いたかわからなくなりがち
...
- ピークアクセスが多い日を選ぶ（いわゆる**五十日**が多い）
- ピーク時間帯前後のリクエストを抽出
  - Cloud Logging / BigQuery を活用
- パスごとに回数を集計 → **パスごとの割合** を算出
- その割合になるよう **シナリオのコードを調整**

→ 「実トラフィックの形」を再現することを重視

<!--
シナリオは実トラフィックベースです。ピーク日のリクエストをCloud LoggingやBigQueryで抽出して、パスごとの割合を出して、それに合うようにシナリオを調整します。
（20秒）
-->
...
- **今**: 専用の閉鎖環境を用意 → シュッと実施可能に
- とはいえ**社内の連携サービス**を含むケースは
  - 先方ステージングを借りる → お願いと周知が必要
...
- **Locust のメトリック保存を忘れない**
  - 終わったあと取り直せないやつ。これ大事。
- APM・各種メトリクスのスクショやエクスポートも忘れず

<!--
実施は計画通りにやるだけ。大事なのはメトリックの保存を忘れないこと。終わったあと取り直せないので。
（10秒）
-->
...
- **レイテンシの分布**
  - 中央値、90%tile、99%tile
- **rps の安定性**
- ユーザシナリオが**正常に最後まで回ったか**
  - 負荷が高すぎると成功したユーザのカウントが上がらない等
- **APM** での内訳
  - どの処理／クエリがボトルネックか？
...
<!--
TODO: あとでいい感じにマスクした実結果を...
（ここも差し替え予定。さっと流す）
-->

---

# 6. ネクストアクションの割り出し (アプリケーション側)

- まず「**今の最大のボトルネック**」を特定する
- レイテンシが大きいエンドポイントを眺める
  - **N+1 がある** → アプリで改善するしかない
...
- 「ネクストアクション」には**アプリの課題が出がち**
  - 問題になるエンドポイントは、結局**そんなに多くない**
  - だからこそ、そのエンドポイント固有のチューニングが効く
  - ときには **仕様に踏み込んだ調整** も必要
- → **ドメインに詳しいバックエンドエンジニア**が
  対応した方が圧倒的にスピードが上がる
